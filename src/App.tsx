...
function Reveal({ children, className = "", direction = "left" }: { children: ReactNode; className?: string; direction?: "left" | "right" }) {
  ...
  useEffect(() => {
    ...
    if (!element) return;
    const threshold = window.innerWidth < 640 ? 0.08 : 0.18;
      ...
      <section id="work" className="border-t border-[var(--ink)]/20 px-5 py-12 sm:px-8 sm:py-20 lg:px-12 lg:py-36">
        ...
        <div className="space-y-16 sm:space-y-24 lg:space-y-44">
          {projects.map((project, index) => (
            <Reveal key={project.no} direction={index % 2 ? "right" : "left"} className="project grid gap-6 lg:grid-cols-12 lg:items-end">
              {/* テキスト：DOM上は常に先頭（モバイルで上表示）、デスクトップは明示配置 */}
              <div className={`pb-1 lg:row-start-1 lg:self-end ${index % 2 ? "lg:col-span-4 lg:col-start-1" : "lg:col-span-3 lg:col-start-10"}`}>
              ...
              {/* 画像：DOM上は2番目（モバイルで下表示）、デスクトップは明示配置 */}
              <Link to={`/work/${project.slug}`} aria-label={`${project.title} のケーススタディを見る`} className={`group relative block overflow-hidden bg-[var(--teal)] lg:row-start-1 ${index % 2 ? "lg:col-span-7 lg:col-start-6" : "lg:col-span-8 lg:col-start-1"}`}>
